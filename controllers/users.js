...
const postUserUUIDByID = async (req, res) => {
  try {
    ...
    console.error(error.message);
    res.json({ status: "error", msg: "error getting user uuid" });
  }
};
...
const postUserNameAndPicByUUID = async (req, res) => {
  try {
    const data = { name: "", profilePicture: "" };
    const allUsers = await UsersModel.find();
    const oneUser = allUsers.filter((user) => user.uuid === req.body.uuid);
    data.name = oneUser[0].name;
    data.profilePicture = oneUser[0].profilePicture;
    res.json(data);
  } catch (error) {
    console.error(error.message);
    res.json({ status: "error", msg: "error getting userdeets" });
  }
};
